...
export function AgentCard({ tag, name, avatar, tagline, model, visibility, isOwner }: AgentCardProps) {
  ...
  return (
    <div className="relative group h-full overflow-hidden">
      <Card className="h-36 md:h-44 overflow-hidden hover:shadow-md transition-all duration-200 border-border hover:border-foreground/20 bg-card p-3 md:p-4 relative">
        <div className="flex flex-col h-full justify-between pr-16 md:pr-20">
          <div className="space-y-1 md:space-y-2">
            ...
            {visibility && visibility !== 'public' && (
              <span className="inline-block px-2 py-0.5 rounded-full bg-muted text-muted-foreground uppercase tracking-wide text-[10px]">
      ...
      {isOwner && (
        <Link
          href={`/edit/${agentId}`}
          className="absolute top-2 right-2 z-10 p-2 text-muted-foreground hover:text-foreground transition-colors opacity-100 md:opacity-0 md:group-hover:opacity-100"
        >
